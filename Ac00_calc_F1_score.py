...
###################################### USING CROSS VALIDATION - USING THE OBSERVER AS A SPLIT ##############
###################################### USING CROSS VALIDATION - USING THE OBSERVER AS A SPLIT ##############
###################################### USING CROSS VALIDATION - USING THE OBSERVER AS A SPLIT ########################
...
# k = 5
# kf_5 = KFold(n_splits=k, shuffle=False)


# results_df = pd.DataFrame(columns=[
#     "GT", "Fold", "Train Accuracy", "Test Accuracy", 
#     "Precision", "Recall", "F1", "RMS", "MAE", "Std Error"
# ])

# for i in range(1, 8):  
#     print(f"Training Random Forest for GT{i}")gt
    
#     y_train = y_train_dict[i].ravel() #.values.ravel()
#     y_test = y_test_dict[i].ravel() #.values.ravel()
...
#     for fold_idx, (train_index, test_index) in enumerate(kf_5.split(X_train)):
...
#         train_accuracy = round(rf.score(X_train_CV, y_train_CV), 4)  # Training accuracy for this fold
#         test_accuracy = round(rf.score(X_test_CV, y_test_CV), 4)    # Validation (test) accuracy for this fold
#         y_pred_CV = rf.predict(X_test_CV)
        
#         precision = round(precision_score(y_test_CV, y_pred_CV, zero_division=0), 4)
#         recall = round(recall_score(y_test_CV, y_pred_CV, zero_division=0), 4)
#         f1 = round(f1_score(y_test_CV, y_pred_CV, zero_division=0), 4)
#         rms = round(mean_squared_error(y_test_CV, y_pred_CV, squared=False), 4)
#         mae = round(mean_absolute_error(y_test_CV, y_pred_CV), 4)
#         std_error = round(np.std(np.abs(y_test_CV - y_pred_CV)), 4)
...
# # Calculate averages for each GT
# average_metrics = results_df.groupby("GT").mean().round(4)
# print("\nAverage Metrics Across All GTs:")
# print(average_metrics)

# # Save the results
# results_df.to_csv("data/train_test_data/testing_features/prAc00_results_bcea_3D_noise_kfold.csv", index=False)
# average_metrics.to_csv("data/train_test_data/testing_features/prAc00_results_GTs_3D_noise_kfold.csv", index=True)

# # Calculate overall average for each metric
# overall_averages = average_metrics.drop(columns=["Fold"]).mean().round(4)

# print("\nOverall Average Metrics Across All GTs:")
# print(overall_averages)

# overall_averages.to_csv("data/train_test_data/testing_features/prAc00_average_results_3D_noise_kfold.csv", header=["Value"], index_label="Metric")
...
#### drop column observer
    ...
    # Example: rf.fit(train_data, y_train)...
